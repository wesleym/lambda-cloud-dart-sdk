...
class SSHKeysApi {
  SSHKeysApi([ApiClient? apiClient]) : apiClient = apiClient ?? defaultApiClient;
  ...
  Future<Response> addSSHKeyWithHttpInfo(AddSSHKeyRequest addSSHKeyRequest,) async {
  ...
  Future<AddSSHKey200Response?> addSSHKey(AddSSHKeyRequest addSSHKeyRequest,) async {
    final response = await addSSHKeyWithHttpInfo(addSSHKeyRequest,);
    ...
    if (response.body.isNotEmpty && response.statusCode != HttpStatus.noContent) {
      return await apiClient.deserializeAsync(await _decodeBodyBytes(response), 'AddSSHKey200Response',) as AddSSHKey200Response;
  ...
  Future<Response> deleteSSHKeyWithHttpInfo(String id,) async {
    // ignore: prefer_const_declarations
    final path = r'/api/v1/ssh-keys/{id}'
      .replaceAll('{id}', id);
  ...
  Future<DeleteSSHKey200Response?> deleteSSHKey(String id,) async {
    final response = await deleteSSHKeyWithHttpInfo(id,);
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
