...
class Filesystem {
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is Filesystem &&
    other.id == id &&
    other.name == name &&
  ...
  @override
  String toString() => 'Filesystem[id=$id, name=$name, mountPoint=$mountPoint, created=$created, createdBy=$createdBy, isInUse=$isInUse, region=$region, bytesUsed=$bytesUsed]';
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "Filesystem[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "Filesystem[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<Filesystem> listFromJson(dynamic json, {bool growable = false,}) {
  ...
  static Map<String, List<Filesystem>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<Filesystem>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = Filesystem.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
