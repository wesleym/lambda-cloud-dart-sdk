...
class FilesystemDeleteResponse {
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is FilesystemDeleteResponse &&
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "FilesystemDeleteResponse[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "FilesystemDeleteResponse[$key]" has a null value in JSON.');
        });
        return true;
      }());
      ...
      return FilesystemDeleteResponse(
        deletedIds: json[r'deleted_ids'] is Iterable
            ? (json[r'deleted_ids'] as Iterable).cast<String>().toList(growable: false)
            : const [],
      );
    }
    return null;
  ...
  static List<FilesystemDeleteResponse> listFromJson(dynamic json, {bool growable = false,}) {
  ...
  static Map<String, List<FilesystemDeleteResponse>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<FilesystemDeleteResponse>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = FilesystemDeleteResponse.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
