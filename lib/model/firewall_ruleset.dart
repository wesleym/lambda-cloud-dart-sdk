...
class FirewallRuleset {
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is FirewallRuleset &&
    other.id == id &&
    other.name == name &&
    other.region == region &&
  ...
  @override
  String toString() => 'FirewallRuleset[id=$id, name=$name, region=$region, rules=$rules, created=$created, instanceIds=$instanceIds]';
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "FirewallRuleset[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "FirewallRuleset[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<FirewallRuleset> listFromJson(dynamic json, {bool growable = false,}) {
  ...
  static Map<String, List<FirewallRuleset>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<FirewallRuleset>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = FirewallRuleset.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
