...
class InstanceActionAvailabilityDetailsReasonCode {
  ...
  /// Returns a new [InstanceActionAvailabilityDetailsReasonCode] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static InstanceActionAvailabilityDetailsReasonCode? fromJson(dynamic value) {
    if (value is Map) {
  ...
  static List<InstanceActionAvailabilityDetailsReasonCode> listFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = InstanceActionAvailabilityDetailsReasonCode.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
