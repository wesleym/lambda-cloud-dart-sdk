...
class InstanceLaunchRequest {
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is InstanceLaunchRequest &&
  ...
  @override
  String toString() => 'InstanceLaunchRequest[regionName=$regionName, instanceTypeName=$instanceTypeName, sshKeyNames=$sshKeyNames, fileSystemNames=$fileSystemNames, fileSystemMounts=$fileSystemMounts, hostname=$hostname, name=$name, image=$image, userData=$userData, tags=$tags, firewallRulesets=$firewallRulesets]';
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "InstanceLaunchRequest[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "InstanceLaunchRequest[$key]" has a null value in JSON.');
        });
        return true;
      }());
      ...
      return InstanceLaunchRequest(
        ...
        sshKeyNames: json[r'ssh_key_names'] is Iterable
            ? (json[r'ssh_key_names'] as Iterable).cast<String>().toList(growable: false)
            : const [],
        fileSystemNames: json[r'file_system_names'] is Iterable
            ? (json[r'file_system_names'] as Iterable).cast<String>().toList(growable: false)
            : const [],
        fileSystemMounts: RequestedFilesystemMountEntry.listFromJson(json[r'file_system_mounts']),
        ...
        firewallRulesets: FirewallRulesetEntry.listFromJson(json[r'firewall_rulesets']),
      );
    }
    return null;
  }

  static List<InstanceLaunchRequest> listFromJson(dynamic json, {bool growable = false,}) {
  ...
  static Map<String, List<InstanceLaunchRequest>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<InstanceLaunchRequest>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = InstanceLaunchRequest.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
