...
class InstanceRestartRequest {
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is InstanceRestartRequest &&
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "InstanceRestartRequest[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "InstanceRestartRequest[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<InstanceRestartRequest> listFromJson(dynamic json, {bool growable = false,}) {
  ...
  static Map<String, List<InstanceRestartRequest>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<InstanceRestartRequest>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = InstanceRestartRequest.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
