...
class InstanceType {
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is InstanceType &&
    other.name == name &&
  ...
  @override
  String toString() => 'InstanceType[name=$name, description=$description, gpuDescription=$gpuDescription, priceCentsPerHour=$priceCentsPerHour, specs=$specs]';
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "InstanceType[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "InstanceType[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<InstanceType> listFromJson(dynamic json, {bool growable = false,}) {
  ...
  static Map<String, List<InstanceType>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<InstanceType>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = InstanceType.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
