...
class InstanceTypeSpecs {
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is InstanceTypeSpecs &&
    other.vcpus == vcpus &&
  ...
  @override
  String toString() => 'InstanceTypeSpecs[vcpus=$vcpus, memoryGib=$memoryGib, storageGib=$storageGib, gpus=$gpus]';
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "InstanceTypeSpecs[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "InstanceTypeSpecs[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<InstanceTypeSpecs> listFromJson(dynamic json, {bool growable = false,}) {
  ...
  static Map<String, List<InstanceTypeSpecs>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<InstanceTypeSpecs>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = InstanceTypeSpecs.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
