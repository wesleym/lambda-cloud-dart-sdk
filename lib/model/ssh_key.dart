...
class SSHKey {
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is SSHKey &&
    other.id == id &&
    other.name == name &&
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "SSHKey[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "SSHKey[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<SSHKey> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <SSHKey>[];
  ...
  static Map<String, List<SSHKey>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<SSHKey>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = SSHKey.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
