...
class TagEntry {
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is TagEntry &&
    other.key == key &&
    other.value == value;
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "TagEntry[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "TagEntry[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<TagEntry> listFromJson(dynamic json, {bool growable = false,}) {
  ...
  static Map<String, List<TagEntry>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<TagEntry>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = TagEntry.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
